...
<!-- blueprint:source-hash f8c69706ab2bbcd9f8d072157c5c0b4b8a48f52fa2971b0cb55a6e147b80fb20 -->
...
In `build-plan.md` order (all MVP items shipped):
...
Post-MVP (next, in order): node text limit 30, design polish pass, IndexedDB storage, then dense text and media nodes, graph cross-links, undo and redo, home duplicate plus search plus JSON import and export, cloud sync and sharing, present mode, dark mode, keyboard nav, PDF and print, ads evaluation.
...
- `text` (string, trimmed, non-empty, max 30 chars) - plain text only, fixed uniform circle (code still enforces the old 60 until build-plan item 8 lands)
...
- **Node text 30 vs shipped 60:** plans now say max 30, but the code still enforces 60 until build-plan item 8 lands. No further decision needed, just build it.
